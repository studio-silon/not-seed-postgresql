...
export class Parser {
    ...
    private heading: Node | null = null;
    ...
    private getParam(separator: string, end: string) {
        ...
        while (this.tokens[this.cursor]) {
            ...
            while (
                ...
                !(
                    this.tokens[this.cursor].type === 'rule' &&
                    (this.tokens[this.cursor].value === '=' || this.tokens[this.cursor].value === separator || this.tokens[this.cursor].value === end)
                )
            ) {
            ...
            while (
                this.tokens[this.cursor] &&
                !(this.tokens[this.cursor].type === 'rule' && (this.tokens[this.cursor].value === separator || this.tokens[this.cursor].value === end))
            ) {
        ...
        while (this.tokens[this.cursor]) {
            ...
            param: while (this.tokens[this.cursor] && this.tokens[this.cursor].type === 'rule') {
                switch (this.tokens[this.cursor].value) {
                    ...
                    case '<|':
                    case '<v|':
                        currentCell.param['vertical-align'] = this.tokens[this.cursor].value === '<^|' ? 'top' : this.tokens[this.cursor].value === '<|' ? 'middle' : 'bottom';
                    ...
                    case '<)>':
                        ...
                        currentCell.param['align'] = this.tokens[this.cursor].value === '<(>' ? 'left' : this.tokens[this.cursor].value === '<|>' ? 'center' : 'right';
                    ...
                    case '<table align=right>':
                        ...
                        node.param['align'] = this.tokens[this.cursor].value.endsWith('left>') ? 'left' : this.tokens[this.cursor].value.endsWith('center>') ? 'center' : 'right';
            ...
            if (currentCell.items.length > 0) currentRow.items.push(currentCell);
    ...
    private walk() {
        ...
        if (token.type === 'rule') {
            if (token.heading) {
                ...
                return node;
            } else if (token.value.startsWith('{{{#!class="')) {
                const node = new Node('Class', {style: token.style || '', items: []});
                ...
                const node = new Node('For', {code: '', value: ''});

                this.cursor++;

                while (this.tokens[this.cursor] && (this.tokens[this.cursor].value === ' ' || this.tokens[this.cursor].value === '\n')) this.cursor++;

                while (this.tokens[this.cursor] && this.tokens[this.cursor].value !== ' ' && this.tokens[this.cursor].value !== '\n') {
                    node.code += this.tokens[this.cursor];
                ...
                const node = new Node('Color', {items: [], color: token.color});
                ...
                const node = new Node('Size', {items: [], size: token.size});
                ...
                return node;
            } else if (token.value === '{{{#!lua') {
                const node = new Node('Lua', {value: ''});
                ...
                return node;
            } else if (token.value === '<?lua' || token.value === '<?lua=') {
                const node = new Node('Lua', {value: token.value === '<?lua=' ? 'return' : ''});
                ...
                while (
                    ...
                    !(
                        this.tokens[this.cursor].type === 'rule' &&
                        (this.tokens[this.cursor].value === '|' || this.tokens[this.cursor].value === ']]' || this.tokens[this.cursor].value === ')]]')
                    )
                ) {
                ...
                while (
                    this.tokens[this.cursor] &&
                    !(this.tokens[this.cursor].type === 'rule' && (this.tokens[this.cursor].value === ']]' || this.tokens[this.cursor].value === ')]]'))
                ) {
                    node.link += this.tokens[this.cursor];
                ...
                while (
                    ...
                    !(this.tokens[this.cursor].type === 'rule' && this.tokens[this.cursor].value === token.value)
                ) {
                    node.items.push(this.walk());
                }

                if (!(this.tokens[this.cursor]?.type === 'rule' && this.tokens[this.cursor].value === token.value)) {
                    this.cursor = _cursor;

                    return new Node('Literal', {value: token.value});
                }
                ...
                while (this.tokens[this.cursor]) {
                    if (this.tokens[this.cursor].type === 'rule' && this.tokens[this.cursor].value === '>') {
                        ...
                        if (this.tokens[this.cursor].value === '\n') {
                            ...
                            if (
                                (inList && this.tokens[this.cursor].value === ' ') ||
                                (this.tokens[this.cursor].value === ' ' && this.tokens[this.cursor + 1]?.type === 'rule' && lists.includes(this.tokens[this.cursor + 1]?.value))
                            ) {
                                if (!inList) {
                                    inList = true;
                ...
                return node;
            } else if (token.value === '[anchor(') {
                const node = new Node('Anchor', {value: ''});
                ...
                if (this.heading) this.heading.value = node.value;
                ...
                return node;
            } else if (token.value === '[목차]' || token.value === '[tableofcontents]' || token.value === '@[toc]') {
                ...
                return node;
            } else if (token.value === '[쓰레드]' || token.value === '[threads]') {
                const node = new Node('Threads', {});
                ...
                if (this.tokens[this.cursor].value === ')](') node.items.push(new Node('Literal', {value: ')'}));
