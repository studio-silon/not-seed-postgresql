...
export class Tokenizer {
    ...
    private isHeadingStart(str: string): Heading | null {
        const start = str.startsWith('======# ')
            ? {depth: 6, folding: true}
            : str.startsWith('====== ')
              ? {depth: 6, folding: false}
              : str.startsWith('=====# ')
                ? {depth: 5, folding: true}
                : str.startsWith('===== ')
                  ? {depth: 5, folding: false}
                  : str.startsWith('====# ')
                    ? {depth: 4, folding: true}
                    : str.startsWith('==== ')
                      ? {depth: 4, folding: false}
                      : str.startsWith('===# ')
                        ? {depth: 3, folding: true}
                        : str.startsWith('=== ')
                          ? {depth: 3, folding: false}
                          : str.startsWith('==# ')
                            ? {depth: 2, folding: true}
                            : str.startsWith('== ')
                              ? {depth: 2, folding: false}
                              : str.startsWith('=# ')
                                ? {depth: 1, folding: true}
                                : str.startsWith('= ')
                                  ? {depth: 1, folding: false}
                                  : null;

        return start;
    }

    private isHeadingEnd(str: string, searchFn: (str: string, search: string) => boolean = (str, search) => str.startsWith(search)): Heading | null {
        const end = searchFn(str, ' #======')
            ? {depth: 6, folding: true}
            : searchFn(str, ' ======')
              ? {depth: 6, folding: false}
              : searchFn(str, ' #=====')
        ...
        root: for (; input.length > this.cursor; this.cursor++) {
            ...
            if (line.startsWith('{{{#!wiki')) {
                ...
                if (line.startsWith('{{{#!wiki style="')) {
                    ...
                    str = line.slice(0, 18 + style.length);
            ...
            let plus: boolean;
            if (
                ((plus = (str = input.slice(this.cursor, this.cursor + 6)).startsWith('{{{+')) || str.startsWith('{{{-')) &&
                str.endsWith(' ') &&
                !isNaN(+(size = str.slice(4, 5)))
            ) {
                this.previousTokenPush();
