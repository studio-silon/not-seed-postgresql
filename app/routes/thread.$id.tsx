        ...
        <Frame>
            <div className="mb-6 bg-white rounded-lg p-4 shadow-sm">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between my-auto gap-2">
                    <div className="flex flex-1 mb-2 sm:mb-0">
                        {isEditing ? (
                            <Form method="post" className="flex gap-2 flex-1" onSubmit={() => setIsEditing(false)}>
                                ...
                                />
                                <Button type="submit" variant="primary" className="size-8 p-0">
                    ...
                    <div className="flex gap-2">
                        ...
                        {canUpdateThread && (
                            <>
                                <Button variant="ghost" className="size-8 p-0" onClick={() => setIsMoving(true)}>
                                    <MoveRight className="h-4 w-4 m-auto" />
                                </Button>
                                <Toggle variant="ghost" className="size-8 p-0" onClick={() => setIsEditing(true)} isActive={isEditing}>
            ...
            <div className="space-y-4 mb-4 max-w-full overflow-hidden">
                {thread.comments.map((comment, index) => (
                    <div
                        key={comment.id}
                        className={`bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition-colors duration-200 w-fit max-w-full break-words ${comment.hidden ? 'opacity-50' : ''}`}
                    >
                        ...
                        <div className="prose prose-sm max-w-full text-gray-600 break-words">{!comment.hidden || canHideComments ? comment.content : ''}</div>
                    </div>
                ))}
            </div>
